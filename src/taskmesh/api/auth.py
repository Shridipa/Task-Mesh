...
def require_role(required_roles: set[str]):
    ...
    async def _check_role(request: Request) -> str:
        ...
        if not x_role:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Missing X-Role header"
            )
        if x_role not in ROLE_HIERARCHY and x_role != "worker":
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail=f"Unknown role: {x_role}"
            )
...
def require_worker_token(worker_secret: str):
    ...
    async def _check_token(request: Request) -> str:
        ...
        if not x_worker_token:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Missing X-Worker-Token header"
            )
        if x_worker_token != worker_secret:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Invalid worker token"
            )
